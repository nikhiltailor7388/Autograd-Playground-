...
- For one-click use, copy `.env.example` to `.env`, put the key in `.env`, and double-click `..\start-infinigrad.bat`. The launcher loads `.env` automatically.

For the one-click Windows launcher, run `..\start-infinigrad.bat` from File Explorer. It starts the frontend, uses the compiled C++ backend when available, and opens the browser.
